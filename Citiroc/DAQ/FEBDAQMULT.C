...
TGTextButton *fTextButton88;
TGTextButton *fTextButton78;

TGNumberEntry * fChanDACt[32];
...
UChar_t bufPMR[1500];
UChar_t bufSCR[1024][1500]; /*A.C. : use a different bufSCR for each board!*/
UChar_t buf[1500];
TH1F * hst[32];
TCanvas *c=0;
TCanvas *c1=0;
...
void PrintConfigNEW(UChar_t *buffer, UShort_t bitlen);

/*A.C. in all these instructions, board is the last byte of the MAC address!!!!*/
void SetThresholdDAC1(int board,UShort_t dac1);
UShort_t GetThresholdDAC1(int board);
void SetThresholdDAC2(int board,UShort_t dac2);
UChar_t ConfigGetGain(int board,int chan);
UChar_t ConfigGetBias(int board,int chan);
UChar_t ConfigGetDACt(int board,int chan);
void ConfigSetGain(int board,int chan, UChar_t val);
void ConfigSetBias(int board,int chan, UChar_t val);
void ConfigSetDACt(int board,int chan, UChar_t val);

Float_t FitPedestal(TH1F *hp);
Float_t FitGain(TH1F *hs);
Double_t x[50], ex[50];
Double_t y[50], ey[50];
...
bool ConfigGetBit(UChar_t *buffer, UShort_t bitlen, UShort_t bit_index,Bool_t verbose=kFALSE)
{
	UChar_t byte;
	UChar_t mask;
	byte=buffer[(bitlen-1-bit_index)/8];
...
void UpdateConfig()
{
	...
	for(int feb=0; feb<t->nclients; feb++)
	{
		...
		if(!(t->ReadBitStream(bsname,bufSCR[t->dstmac[5]]))) t->ReadBitStream("CITIROC_SC_PROFILE1.txt",bufSCR[t->dstmac[5]]);
		//if(!(t->ReadBitStream(bsname,bufSCR))) t->ReadBitStream("CITIROC_SC__FEB050_FNALSCINTI.txt",bufSCR);

		t->SendCMD(t->dstmac,FEB_SET_RECV,fNumberEntry75->GetNumber(),t->srcmac);
		t->SendCMD(t->dstmac,FEB_WR_SCR,0x0000,bufSCR[t->dstmac[5]]);
		t->SendCMD(t->dstmac,FEB_WR_PMR,0x0000,bufPMR);

		if(feb==BoardToMon)  {
			for(int i=265; i<265+32;i++)
				if(ConfigGetBit(bufSCR[t->dstmac[5]],1144,i)) fChanEnaTrig[i-265]->SetOn(); else fChanEnaTrig[i-265]->SetOn(kFALSE);
			if(ConfigGetBit(bufSCR[t->dstmac[5]],1144,1139)) fChanEnaTrig[32]->SetOn(); else fChanEnaTrig[32]->SetOn(kFALSE);
			for(int i=0; i<32;i++)
				if(ConfigGetBit(bufSCR[t->dstmac[5]],1144,633+i*15)) fChanEnaAmp[i]->SetOn(kFALSE); else fChanEnaAmp[i]->SetOn();
			...
			fNumberEntry755->SetNumber(GetThresholdDAC1(t->dstmac[5]));
			for(int i=0; i<32;i++) fChanGain[i]->SetNumber(ConfigGetGain(t->dstmac[5],i));
			for(int i=0; i<32;i++) fChanBias[i]->SetNumber(ConfigGetBias(t->dstmac[5],i));
			for(int i=0; i<32;i++) fChanDACt[i]->SetNumber(ConfigGetDACt(t->dstmac[5],i));
		}
	}
	//fNumberEntry755->SetNumber();
}

UChar_t ConfigGetGain(int boardMAC,int chan)
{
	UChar_t val=0;
	for(int b=0;b<6;b++)
	{
		val=val << 1;
		if(ConfigGetBit(bufSCR[boardMAC],1144,619+chan*15+b)) val=val+1;
	}
	return val;
}


UChar_t ConfigGetDACt(int boardMAC,int chan)
{
	UChar_t val=0;
	...
	{
		val=val << 1;
		if(ConfigGetBit(bufSCR[boardMAC],1144,0+chan*4+b)) val=val+1;
	}
	return val;
}

UChar_t ConfigGetBias(int boardMAC,int chan)
{
	UChar_t val=0;
	for(int b=0;b<8;b++)
	{
		val=val << 1;
		if(ConfigGetBit(bufSCR[boardMAC],1144,331+chan*9+b)) val=val+1;
	}
	return val;
}

void ConfigSetGain(int boardMAC,int chan, UChar_t val)
{
	UChar_t mask=1<<5;
	for(int b=0;b<6;b++)
	{
		if((val & mask)>0) ConfigSetBit(bufSCR[boardMAC],1144,619+chan*15+b,kTRUE); else ConfigSetBit(bufSCR[boardMAC],1144,619+chan*15+b,kFALSE);
		mask=mask>>1;
	}

}

void ConfigSetBias(int boardMAC,int chan, UChar_t val)
{
	UChar_t mask=1<<7;
	for(int b=0;b<8;b++)
	{
		if((val & mask)>0) ConfigSetBit(bufSCR[boardMAC],1144,331+chan*9+b,kTRUE); else ConfigSetBit(bufSCR[boardMAC],1144,331+chan*9+b,kFALSE);
		mask=mask>>1;
	}
...
void ConfigSetDACt(int boardMAC,int chan, UChar_t val)
{
	UChar_t mask=1<<3;
	for(int b=0;b<4;b++)
	{
		if((val & mask)>0) ConfigSetBit(bufSCR[boardMAC],1144,0+chan*4+b,kTRUE); else ConfigSetBit(bufSCR[boardMAC],1144,0+chan*4+b,kFALSE);
		mask=mask>>1;
	}
...
void SendConfig()
{
	...
	SetDstMacByIndex(BoardToMon); /*A.C. set the dst mac to the currently monitored board*/


	for(int i=265; i<265+32;i++)
		if(fChanEnaTrig[i-265]->IsOn()) ConfigSetBit(bufSCR[t->dstmac[5]],1144,i,1); else  ConfigSetBit(bufSCR[t->dstmac[5]],1144,i,0);
	if(fChanEnaTrig[32]->IsOn()) ConfigSetBit(bufSCR[t->dstmac[5]],1144,1139,1); else  ConfigSetBit(bufSCR[t->dstmac[5]],1144,1139,0);

	for(int i=0; i<32;i++)
		if(fChanEnaAmp[i]->IsOn()) ConfigSetBit(bufSCR[t->dstmac[5]],1144,633+i*15,0); else  ConfigSetBit(bufSCR[t->dstmac[5]],1144,633+i*15,1);

	for(int i=0; i<32;i++)
		if(fChanProbe[i]->IsOn()) ConfigSetBit(bufPMR,224,96+i,1); else  ConfigSetBit(bufPMR,224,96+i,0);

	for(int i=0; i<32;i++) ConfigSetBias(t->dstmac[5],i, fChanBias[i]->GetNumber());
	for(int i=0; i<32;i++) ConfigSetGain(t->dstmac[5],i, fChanGain[i]->GetNumber());
	for(int i=0; i<32;i++) ConfigSetDACt(t->dstmac[5],i, fChanDACt[i]->GetNumber());
...
void SendAllChecked()
{
	...
	{
		fChanEnaAmp[i]->SetOn();
		ConfigSetBit(bufSCR[t->dstmac[5]],1144,633+i*15,0);
	}
...
void SendAllUnChecked()
{
	...
	for(int i=0; i<32;i++)
	{
		...
		ConfigSetBit(bufSCR[t->dstmac[5]],1144,633+i*15,1);
	}
...
int Init(const char *iface="eth1")
{
	...
	for(int i=0;i<256;i++) gts0ref[i]=new TGraph();
	...
	while(jj<truncat-18)
	{
		...
		for(kk=0; kk<32; kk++) if (CHAN_MASK & (1<<kk))
		{
			...
			{ 
				...
				if(fUpdateHisto->IsOn() && fTab683->GetCurrent()==6) {
					hevdisp->SetBinContent(kk+1,NEVDISP,adc);
				}
			}
			chg[kk]=adc;
		...
		if(t->dstmac[5] == t->macs[BoardToMon][5])
			if(fUpdateHisto->IsOn() && fTab683->GetCurrent()==6) {
				...
				{
					hevdisp->SetBinContent(kk+1,evi-1,hevdisp->GetBinContent(kk+1,evi));
				}
			}
		...
		gts0ref[mac5]->SetPoint(gts0ref[mac5]->GetN(),gts0ref[mac5]->GetN(),ts0_ref-1e9);
		if(ts1!=0) gts1[mac5]->SetPoint(gts1[mac5]->GetN(),gts1[mac5]->GetN(),ts1);
		if(ts0!=0) gts0[mac5]->SetPoint(gts0[mac5]->GetN(),gts0[mac5]->GetN(),ts0);
		tr->Fill();
		if(t->dstmac[5] == t->macs[BoardToMon][5])
		{
			evs++;
...
void UpdateHisto()
{
	...
	c1->Update();
	c3->cd(1);
	gts0ref[t->macs[0][5]]->Draw("AL");
	gts0ref[t->macs[0][5]]->GetHistogram()->GetYaxis()->SetRangeUser(-100,100);
	gts0ref[t->macs[0][5]]->GetHistogram()->GetXaxis()->SetTitle("Event number");
	gts0ref[t->macs[0][5]]->GetHistogram()->GetYaxis()->SetTitle("TS0 period deviation from 1s, ns");
	for(int feb=0; feb<t->nclients; feb++)  { gts0ref[t->macs[feb][5]]->Draw("sameL"); }
	c3->cd(2);
	gts1[t->macs[0][5]]->SetLineColor(kRed);
	gts1[t->macs[0][5]]->Draw("AL");
	gts1[t->macs[0][5]]->GetHistogram()->GetXaxis()->SetTitle("Event number");
	gts1[t->macs[0][5]]->GetHistogram()->GetYaxis()->SetTitle("TS0, TS1 (red), ns");

	//gts0ref[t->macs[0][5]]->GetHistogram()->GetYaxis()->SetRangeUser(-100,100);
	for(int feb=0; feb<t->nclients; feb++)  { gts1[t->macs[feb][5]]->SetLineColor(kRed); gts1[t->macs[feb][5]]->Draw("sameL"); gts0[t->macs[feb][5]]->Draw("sameL"); }
	c3->Update();
	c4->cd();
	hcprof->Draw("hist");
	c4->Update();
...
void DAQ(int nev=0)
{
	...
	while(RunOn==1 && (nevv==0 || evs<nevv) && (fNumberEntryTME->GetNumber()==0 || tm1-tm0 < fNumberEntryTME->GetNumber() ))
	{
		...
		if(fUpdateHisto->IsOn() && fTab683->GetCurrent()==2) {
			c1->cd(); hst[chan]->Draw(); // FitGain(hst[chan]);
			c1->Update();
		}
...
void Reset()
{
	...
	evs=0;
	total_lost=0;
	for(int feb=0; feb<t->nclients; feb++)  { gts0ref[t->macs[feb][5]]->Set(0); }
	for(int feb=0; feb<t->nclients; feb++)  { gts1[t->macs[feb][5]]->Set(0); gts0[t->macs[feb][5]]->Set(0); }
	c3->cd(1);
	//    hcprof->Draw("hist");
	c3->Update();
...
void HVON()
{
	...
	fTextButton88->SetEnabled(1);
	fTextButton78->SetEnabled(0);
...
void HVOF()
{
	...
	fTextButton88->SetEnabled(0);
	fTextButton78->SetEnabled(1);
...
void ThresholdScan(UShort_t start, UShort_t stop)
{
	...
	{
		SetThresholdDAC1(t->dstmac[5],thr);
		SetThresholdDAC2(t->dstmac[5],thr);
		gSystem->Sleep(2000); //to let FEB update rate
		rate=GetTriggerRate();
		printf("Rate %f at threshold %d\n",rate,thr);
		while((rate<=1 || rate>50000) && retakes<10) {rate=GetTriggerRate(); retakes++; printf("Rate out of range (1,50000), retry attempt %d..\n",retakes);}
		retakes=0;
		if(rate<=1 || rate>50000) {printf("Rate out of range (1,50000), skipping point.\n");}
		else { gr->SetPoint(i,thr,rate); i++;}
		c1->cd(); gr->Draw("AL");
		gr->GetHistogram()->GetXaxis()->SetTitle("Threshold, DAC units");
		gr->GetHistogram()->GetYaxis()->SetTitle("Event rate, Hz");
		gr->GetHistogram()->SetTitle("Event rate vs threshold");
		c1->Update();
	}
}
...
void SetThresholdDAC1(int board,UShort_t dac1)
{
	...
	for(int i=0; i<10;i++)
	{
		if( (dac1 & 1)>0) ConfigSetBit(bufSCR[board],1144,offset+9-i,kTRUE);
		else ConfigSetBit(bufSCR[board],1144,offset+9-i,kFALSE);
		dac1= dac1 >> 1;
	}
	...
	printf("Setting threshold for mac5=%d\n",t->dstmac[5]);
	t->SendCMD(t->dstmac,FEB_WR_SCR,0x0000,bufSCR[board]);
}
...
UShort_t GetThresholdDAC1(int board)
{
	...
	{
		dac1= dac1 >> 1;
		if(ConfigGetBit(bufSCR[board],1144,offset+9-i)) dac1=dac1 | 0x0200;
	}
	return dac1;
}

void SetThresholdDAC2(int board,UShort_t dac1)
{
	t->dstmac[5]=board;
...
void GUI_UpdateThresholdAll()
{
	UShort_t dac1;
	dac1=fNumberEntry755->GetNumber();
	BoardToMon=fNumberEntry8869->GetNumber();

	for (int board=0;board< t->nclients; board++){
		t->dstmac[5]=t->macs[board][5];
		SetThresholdDAC1(t->dstmac[5],dac1);
		SetThresholdDAC2(t->dstmac[5],dac1);
	}
...
void UpdateBoardMonitor()
{
	...
	for(int i=0; i<32;i++) fChanGain[i]->SetNumber(ConfigGetGain(t->macs[BoardToMon][5],i));
	for(int i=0; i<32;i++) fChanBias[i]->SetNumber(ConfigGetBias(t->macs[BoardToMon][5],i));
	for(int i=0; i<32;i++) fChanDACt[i]->SetNumber(ConfigGetDACt(t->macs[BoardToMon][5],i));
...
void FEBGUI()
{
	...
	fTextButton78 = new TGTextButton(fGroupFrame679,"SiPM HV ON");
	...
	fTextButton88 = new TGTextButton(fGroupFrame679,"SiPM HV OFF");
	...
	TGTextButton *fTextButton8819 = new TGTextButton(fGroupFrame679,"Fit peaks");
	fTextButton8819->SetTextJustify(36);
	fTextButton8819->SetMargins(0,0,0,0);
	fTextButton8819->SetWrapLength(-1);
	fTextButton8819->Resize(123,22);
	fGroupFrame679->AddFrame(fTextButton8819, new TGLayoutHints(kLHintsLeft| kLHintsCenterX  | kLHintsTop | kLHintsExpandX,0,0,2,2));
	fTextButton8819->SetCommand("MakeFitPeaks()");
	...
	TGTextButton *fTextButton931 = new TGTextButton(fGroupFrame679,"Set Threshold all");
	...
	fTextButton931->SetCommand("GUI_UpdateThresholdAll()");
	fGroupFrame679->AddFrame(fTextButton931, new TGLayoutHints(kLHintsLeft | kLHintsTop,0,0,2,2));

	TGTextButton *fTextButton9319 = new TGTextButton(fGroupFrame679,"Set Threshold Moned");
	fTextButton9319->SetTextJustify(36);
	fTextButton9319->SetMargins(0,0,0,0);
	fTextButton9319->SetWrapLength(-1);
	fTextButton9319->Resize(123,22);
	fTextButton9319->SetCommand("GUI_UpdateThreshold()");
	fGroupFrame679->AddFrame(fTextButton9319, new TGLayoutHints(kLHintsLeft | kLHintsTop,0,0,2,2));
	...
	fNumberEntry886->Connect("ValueSet(Long_t)", 0, 0,  "UpdateADCHisto()");
   ...
   fGroupFrame679->AddFrame(fLabel, new TGLayoutHints(kLHintsLeft  | kLHintsExpandX,0,0,39,0));
	...
	TGGroupFrame* fGains=new TGGroupFrame(fCompositeFrame686,"HG preamp gain/bias/DACt thr");
	//fGains->SetLayoutManager(new TGVerticalLayout(fGains));
	fGains->SetLayoutManager(new TGMatrixLayout(fGains, 32, 4, 0, kLHintsLeft | kLHintsTop));
	for(int i=0;i<32;i++)
	{
		sprintf(str," CH %d",i);
		...
		fChanDACt[i] = new TGNumberEntry(fGains, (Double_t) i,4,-1,(TGNumberFormat::EStyle) 0,(TGNumberFormat::EAttribute) 1,(TGNumberFormat::ELimit) 2,0,15);
		fChanDACt[i]->Connect("ValueSet(Long_t)", 0, 0,  "SendConfig()");
		fChanDACt[i]->SetHeight(20);

		fGains->AddFrame(new TGLabel(fGains,str));
		fGains->AddFrame(fChanGain[i]);
		fGains->AddFrame(fChanBias[i]);
		fGains->AddFrame(fChanDACt[i]);
	...
	// container of "PeakFit"
	TGCompositeFrame *fCompositeFrame735221;
	fCompositeFrame735221 = fTab683->AddTab("Peaks fit");
	fCompositeFrame735221->SetLayoutManager(new TGVerticalLayout(fCompositeFrame735221));

	// embedded canvas
	TRootEmbeddedCanvas *fRootEmbeddedCanvas736221 = new TRootEmbeddedCanvas(0,fCompositeFrame735221,1179,732+100);
	Int_t wfRootEmbeddedCanvas736221 = fRootEmbeddedCanvas736221->GetCanvasWindowId();
	c7 = new TCanvas("c7", 10, 10, wfRootEmbeddedCanvas736221);
	fRootEmbeddedCanvas73622->AdoptCanvas(c7);
	fCompositeFrame735221->AddFrame(fRootEmbeddedCanvas736221, new TGLayoutHints(kLHintsLeft | kLHintsTop | kLHintsExpandX | kLHintsExpandY,2,2,2,2));


	fTab683->SetTab(0); /*Configuration tab*/
...
void AcquireGainCalibrationData(int nevc=10000)
{
	...
	for(int sci=0; sci<16; sci++)
	{
		...
		{
			fChanEnaAmp[i]->SetOn(kFALSE);
			ConfigSetBit(bufSCR[t->macs[BoardToMon][5]],1144,633+i*15,1);
		}
		i=sci*2;
		fChanEnaAmp[i]->SetOn();
		ConfigSetBit(bufSCR[t->macs[BoardToMon][5]],1144,633+i*15,0);
		i++;
		fChanEnaAmp[i]->SetOn();
		ConfigSetBit(bufSCR[t->macs[BoardToMon][5]],1144,633+i*15,0);
		t->SendCMD(t->dstmac,FEB_WR_SCR,0x0000,bufSCR[t->macs[BoardToMon][5]]);
