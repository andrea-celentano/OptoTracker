...
void SendConfig()
{
  ...
  if(fChanEnaTrig[32]->IsOn()) ConfigSetBit(bufSCR,1144,1139,1); else  ConfigSetBit(bufSCR,1144,1139,0);
...
void SetThresholdDAC1(UShort_t dac1)
{
printf("Setting threshold for mac5=%d to value=%i\n",t->dstmac[5],dac1);
int offset=1107;
for(int i=0; i<10;i++)
{ 
   ...
   void GUI_UpdateThresholdAll()
    {
     ...
     printf("GUI_UpdateThresholdAll to value: %i\n",dac1);
  ...
  void GUI_UpdateThreshold()
    {
     UShort_t dac1;
     dac1=fNumberEntry755->GetNumber();
     printf("GUI_UpdateThreshold to value: %i\n",dac1);
