	...
	for (int iPMT=0;iPMT<6;iPMT++){
		...
		for (int id=0;id<m_detector->getNPixels(iPMT);id++){
			switch (m_fitLikelihoodMode){
				case(k_onlyCharge):
					pQ=PointLikelihoodCharge(iPMT,id,x);/*This already returns the log of the charge probability*/
					break;
				case (k_onlyTime):
					...
					else pT=PointLikelihoodTime(iPMT,id,x);   /*This already returns the log of the time probability*/
					break;
				case (k_both):
					Nphotons=m_N[iPMT][id]; //measured number of photons in this pixel
					pQ=PointLikelihoodCharge(iPMT,id,x); /*This already returns the log of the charge probability*/
...
double TRecon::PointLikelihoodTime(int iPMT,int id,const double *para) const{
...
*/
double TRecon::PointLikelihoodCharge(int iPMT,int id,const double *para) const{
	...
	int Nphe=m_N[iPMT][id];     //hit number of photo-electrons
	double t=m_t[iPMT][id];     //hit time, useless here
	double mu0,ret,eps;
	TVector3 xp=m_detector->getPosPixel(iPMT,id); //pixel position
	...
	mu0=SinglePixelAverageCharge(x0,iPMT,id);	
	mu0*=N0;
	//cout<<mu0<<endl;
	//cin.get();
