...
void TOpNoviceSelectorRaw::SlaveBegin(TTree * /*tree*/)
{
	...
	for (int ii=0;ii<6;ii++){
		...
		for (int jj=0;jj<m_detector->getNdet(ii);jj++){
			q[ii][jj]=new double[m_detector->getNPixels(ii,jj)]();
			N[ii][jj]=new int[m_detector->getNPixels(ii,jj)]();
			tFirst[ii][jj]=new double[m_detector->getNPixels(ii,jj)]();
		}
	}
	...
	for (int ii=0;ii<6;ii++){
		for (int jj=0;jj<m_detector->getNdet(ii);jj++){
			if (m_detector->isDetPresent(ii,jj)){
				hPixel0[ii][jj]=new TH2D(Form("hPixel0_%i_%i",ii,jj),Form("hPixel0_%i_%i",ii,jj),m_detector->getNPixelsX(ii,jj),-m_detector->getDetSizeX(ii,jj)/2,m_detector->getDetSizeX(ii,jj)/2,m_detector->getNPixelsY(ii,jj),-m_detector->getDetSizeY(ii,jj)/2,m_detector->getDetSizeY(ii,jj)/2);
				hPixel0[ii][jj]->SetTitle(hPixel0Title[ii].c_str());
				fOutput->Add(hPixel0[ii][jj]); //VERY IMPORTANT: fill output list


				int Nx=m_detector->getNPixelsX(ii,jj);
				int Ny=m_detector->getNPixelsY(ii,jj);
				for (int id=0;id<Nx*Ny;id++){
					hCharge[ii][jj].push_back(new TH1D(Form("hCharge_%i_%i_%i",ii,jj,id),Form("hCharge_%i_%i_%i",ii,jj,id),100,-0.5,99.5));fOutput->Add(hCharge[ii][jj].at(id));
					hTime[ii][jj].push_back(new TH1D(Form("hTime_%i_%i_%i",ii,jj,id),Form("hTime_%i_%i_%i",ii,jj,id),400,-2.5,17.5));fOutput->Add(hTime[ii][jj].at(id));
					hTimeVsCharge[ii][jj].push_back(new TH2D(Form("hTimeVsCharge_%i_%i_%i",ii,jj,id),Form("hTimeVsCharge_%i_%i_%i",ii,id),100,-0.5,99.5,400,-2.5,17.5));fOutput->Add(hTimeVsCharge[ii][jj].at(id));
				}
			}
		}
	}
...
Bool_t TOpNoviceSelectorRaw::Process(Long64_t entry)
{
	...
	for (int ii=0;ii<6;ii++){
		for (int jj=0;jj<m_detector->getNdet(ii);jj++){
			for (int ipid=0;ipid<m_detector->getNPixels(ii,jj);ipid++){
				N[ii][jj][ipid]=0;
				q[ii][jj][ipid]=0;
	...
	for (int ihit=0;ihit<Nhit;ihit++){
		...
		if (m_detector->isDetPresent(iface,idetector)==0){
		...
		for (int iphe=0;iphe<Nphe;iphe++){
			...
			if (m_detector->getDetTimeRes(iface,idetector)>0) t=t+m_rand->Gaus(0,m_detector->getDetTimeRes(iface,idetector));
	...
	for (int ii=0;ii<6;ii++){
		for (int jj=0;jj<m_detector->getNdet(ii);jj++){
			...
			for (int id=0;id<m_detector->getNPixels(ii,jj);id++){
				hCharge[ii][jj].at(id)->Fill(N[ii][jj][id]);
				hTime[ii][jj].at(id)->Fill(tFirst[ii][jj][id]);
				hTimeVsCharge[ii][jj].at(id)->Fill(N[ii][jj][id],tFirst[ii][jj][id]);
			}
		}
	}
	...
	for (int ii=0;ii<6;ii++){
		for (int jj=0;jj<m_detector->getNdet(ii);jj++){
			if (m_detector->isDetPresent(ii,jj)==0) continue;
			m_recon->setNaddress(ii,jj,N[ii][jj]);
			m_recon->setTaddress(ii,jj,tFirst[ii][jj]);
		}
	}
	//compute COG
