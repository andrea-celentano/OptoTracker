...
/*This function returns the minimal propagation time for a track between x0 and x1, moving with speed b
 *
 */
double TRecon::GetMinimalDelay(const TVector3 &x0,const TVector3 &x1,double beta,int iface,int idetector,int id) const{
...
double TRecon::TrackLikelihoodTime(int iface,int idetector,int id,const double *para) const{
	...
	tmin=this->GetMinimalDelay(x0,x1,b,iface,idetector,id);
