...
//The experimental hall walls are all 100cm away from housing walls
	...
	for (int ii=0;ii<6;ii++){
		...
		if ((fPhotoDetectorSizeX[ii]>0)&&(fPhotoDetectorSizeY[ii]>0)){
			fDetector_box[ii]=new G4Box("Detector",fPhotoDetectorSizeX[ii]/2,fPhotoDetectorSizeY[ii]/2,detectorThickness/2);
			fDetector_log[ii]=new G4LogicalVolume(fDetector_box[ii],fAir,"Detector",0,0,0);
			
			
			fPixel_box[ii]=new G4Box("Pixel",fPixelSizeX[ii]/2,fPixelSizeY[ii]/2,pixelThickness/2);
			fPixel_log[ii]=new G4LogicalVolume(fPixel_box[ii],fAir,"Pixel",0,0,0);
			
			VisAtt = new G4VisAttributes(G4Colour(0.1,1.,0.1,0.8));
			VisAtt->SetForceSolid(true);
			fDetector_log[ii]->SetVisAttributes(VisAtt);
			new G4PVPlacement(0,G4ThreeVector(0,0,faceThickness/2-fCouplingThickness[ii]-detectorThickness/2),fDetector_log[ii],Form("Detector_%i",ii),fFace_log[ii],false,ii);//I use a copy-number to keep the face number
