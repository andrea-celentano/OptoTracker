...
TDetector::TDetector(string fname):
...
{	
	...
	m_name="DefaultDetector";
	if (!file){
		cout<<"Error in TDetector, file not found: "<<fname<<endl;
		exit(1);
	}
	while(!file.eof()){
		getline (file,line);
...
void TDetector::PrintPixels(Option_t* option) const{
	int id;
...
void  TDetector::Print(Option_t* option) const{
	printf("Detector :%s \n",m_name.c_str());
...
TVector3 TDetector::getPosPixel(int iface,int idetector,int ipixel) const{
	TVector3 vp(posPixelX[iface][idetector][ipixel],posPixelY[iface][idetector][ipixel],posPixelZ[iface][idetector][ipixel]);
	return vp;
}
...
TVector3 TDetector::getFaceNormal(int iface) const{
	switch (iface){
	case 0:
...
TVector3 TDetector::getFaceT1(int iface) const{
	switch (iface){
	case 0:
		return TVector3(-1.,0.,0.);
		break;
...
TVector3 TDetector::getFaceT2(int iface) const{
	switch (iface){
	case 0:
		return TVector3(0.,1.,0.);
		break;
...
/*Return the first tangent vector*/
TVector3 TDetector::getDetectorT1(int iface,int idetector) const{
...
/*Return the second tangent vector*/
TVector3 TDetector::getDetectorT2(int iface,int idetector) const{
...
int TDetector::getDetGlobalID(int iface,int idetector) const{
	int ret=0;
...
void TDetector::getFaceDetIDfromGlobal(int global,int &iface,int &idetector) const{
	int iTmp=global;
	for (int ii=0;ii<6;ii++){
...
int TDetector::getPixelGlobalID(int iface,int idetector,int ipixel) const{
	int ret=0;
	for (int ii=0;ii<iface;ii++){
...
void TDetector::getFaceDetPixelIDfromGlobal(int global,int &iface,int &idetector,int &ipixel) const{
	int iTmp=global;
	for (int ii=0;ii<6;ii++){
		for (int jj=0;jj<this->getNdet(ii);jj++){
