	...
	for (int ii = 0; ii < m_fileName->size(); ii++) {
		...
		while ((key = (TKey*) next())) {
			if (string(key->GetClassName()) == "TTree")
				continue;
			else if (string(key->GetClassName()) == "TChain")
				continue;
			else if (string(key->GetClassName()) == "TDetector")
				continue;
			else {
...
void TAnalysis::run() {
	 ...
	 */
	if (iterationN == 0) {
		if (m_manager->getNumberOfEvents() == -1) {
			m_manager->setNumberOfEvents(m_chain->GetEntries());
		} else if (m_manager->getNumberOfEvents() > m_chain->GetEntries()) {
			m_manager->setNumberOfEvents(m_chain->GetEntries());
		}
		Info("run", "Chain has :%i events", m_chain->GetEntries());
		Info("run", "Manager has %i events", m_manager->getNumberOfEvents());
	}
	for (iterationN = 0; iterationN < m_manager->getNumberOfIterations(); iterationN++) {
		...
		if ((m_manager->getVerboseLevel() >= TJobManager::normalVerbosity) && output) {
			...
			while (TObject *obj = outputIter->Next())
				Info("run", "Object %s is in next input", obj->GetName());
			delete outputIter;
		}
