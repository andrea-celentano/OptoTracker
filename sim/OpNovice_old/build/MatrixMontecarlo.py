...
workDir="/project/Gruppo3/fiber5/celentano/OptoTracker/sim/OpNovice_old/build/"
saveDir="run7"
matrixDir="matrix0"
#Executables
baseExe="/project/Gruppo3/fiber5/celentano/OptoTracker/sim/OpNovice_old/build/OpNoviceExe" 
...
for iz in range(0,Nz):
    for iy in range(0,Ny):
	    ...
	    if (doGeant):
		runFile.write(baseExe+" -m "+macroFileName+"\n");#launch MC
	    ...
	    runFile.write("cd ..\n");
            ...
            if (doFarm):
                if (len(resources)>0):
                    arg="-q "+queue_name+" -P "+arch_name+" -R \""+resources+"\" -e log"+str(ibin)+".e"+" -o log"+str(ibin)+".o "+runFileName
                else:
                    arg="-q "+queue_name+" -P "+arch_name+" -e log"+str(ibin)+".e"+" -o log"+str(ibin)+".o "+runFileName
