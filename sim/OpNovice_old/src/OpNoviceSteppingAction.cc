...
//This is a primary track
	...
	if(particleType==G4OpticalPhoton::OpticalPhotonDefinition() ) {
		...
		if (thePostPV->GetName()=="H8500GlassInt"){
			fRootIO->fillHistogram1D(1,thePostPoint->GetPosition().y()/cm,1.);	
			
			G4ThreeVector globalPosition=thePostPoint->GetPosition();
			G4ThreeVector localPosition=thePostPoint->GetTouchableHandle()->GetHistory()->GetTopTransform().TransformPoint(globalPosition);
			
			fRootIO->fillHistogram2D(2,localPosition.x()/cm,localPosition.y()/cm,1.);	
		}
